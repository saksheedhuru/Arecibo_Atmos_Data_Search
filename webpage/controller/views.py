...
# if request.method == "POST":
    ...
    distinct_wavelengths_query = "SELECT DISTINCT(Wavelength) FROM Sky_Imager WHERE Wavelength > 0;"
...
# <a href="?location=arecibo&amp;year=2023&amp;filt=5577&amp;month=Mar&amp;day=17">17</a>
    ...
    for key in months_dict.keys():
        ...
        distinct_day_location_wavelength = distinctDayOfMonth(set_year, month_num, set_location, set_wavelength, table_name)
        new_days = []
...
def interferometer_calendar(request):
    mydb = mysql.connector.connect(
    host="localhost",
    user="atmos",
    password=password,
    ...
    # if request.method == "POST":
    table_name = "Interferometer"

    cursor = mydb.cursor(dictionary=True)
    # Define query
    distinct_years_query = "SELECT DISTINCT(Year) FROM Interferometer;"
    cursor.execute(distinct_years_query)

    distinct_years_results = cursor.fetchall()

    distinct_locations_query = "SELECT DISTINCT(Location) FROM Interferometer;"
    cursor.execute(distinct_locations_query)

    distinct_locations_results = cursor.fetchall()

    distinct_wavelengths_query = "SELECT DISTINCT(Wavelength) FROM Interferometer WHERE Wavelength > 0;"
    cursor.execute(distinct_wavelengths_query)
    ...
    for wavelength in wavelengths:
        ...
        selected = False
        # Check if the current wavelength being looked at is the same as set wavelength
        if wavelength == str(set_wavelength):
            selected = True
        wavelengths_TF.append([wavelength, selected])
...
# <a href="?location=arecibo&amp;year=2023&amp;filt=5577&amp;month=Mar&amp;day=17">17</a>
    ...
    for key in months_dict.keys():
        ...
        for day in context[key]:
            ...
            # The href woulkd be: interferometer_files?year={year_value}&month={month_value}&day={day_value}&location={location_value}
            # Start by just changing it to interferometer_files

            # Assigning True or False as to whether it has data or not
            if day in distinct_day_location_wavelength:
                new_element = [day, True, f"interferometer_files?year={set_year}&month={month_num}&day={day}&location={set_location}&wavelength={set_wavelength}"]
            else:
                new_element = [day, False, "https://www.google.com/"]
...
def distinctDayOfMonth(year, month, location, wavelength, table_name):
    mydb = mysql.connector.connect(
    host="localhost",
    user="atmos",
    password=password,
    ...
    cursor = mydb.cursor(dictionary=True)
    available_data_query = f"SELECT distinct Day FROM {table_name} WHERE Year = '{year}' AND Month = '{month}' AND Location = '{location}' AND Wavelength  = '{wavelength}';"
